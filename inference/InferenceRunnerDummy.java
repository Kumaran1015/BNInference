...
public class InferenceRunner {
	...
	InferenceRunner() throws IOException, InterruptedException, ClassNotFoundException{
		...
		FSDataInputStream fsdis = fs.open(new Path("/user/training/junction_tree/final_output/part-r-00000")); 
		...
		*/
		...
		for(int i =0;i<jtnodes.size();i++){
			...
			Path src = new Path("/user/training/FYP/"+file_name+"/file.seq");
			Path dst = new Path("/user/training/FYP/"+file_name+"_copy/file.seq");
			if(fs.exists(new Path("/user/training/FYP/"+file_name+"_copy")))
				fs.delete((new Path("/user/training/FYP/"+file_name+"_copy")),true);
		...
		for(int i=0;i<jtnodes.size();i++)
			visited.put(jtnodes.get(i).name , false);
		if(fs.exists(new Path("/user/training/FYP/InferenceFinalOutput")))
			fs.delete(new Path("/user/training/FYP/InferenceFinalOutput"), true);
		FSDataOutputStream fsdos = fs.create(new Path("FYP/InferenceFinalOutput/"+changeName(root.name)));
		FileStatus[] fss = fs.listStatus(new Path("/user/training/FYP/"+changeName(root.name)+"_copy"),new PathFilter() {
			  @Override
		...
		//Generating Output.
		Path outputPath = new Path("/user/training/FYP/InferenceFinalOutput");
		buildOutput(outputPath);
		
	}
	
	public HashMap<String,String> parse(String jsonLine) {
	...
	private void buildOutput(Path outputPath) throws IOException, InterruptedException, ClassNotFoundException {
		...
		FileOutputFormat.setOutputPath(job, new Path("/user/training/FYP/OutputDump"));
		//job.setInputFormatClass(TextInputFormat.class);
		//job.setOutputFormatClass(TextOutputFormat.class);
		
		if (fs.exists(new Path("/user/training/FYP/OutputDump")))
			fs.delete(new Path("/user/training/FYP/OutputDump"), true);
	...
	private void evidenceDistribution(JTNode root) throws IOException, InterruptedException, ClassNotFoundException{
		...
		if(len > 0){
			...
			FileInputFormat.addInputPath(job, new Path("/user/training/FYP/"+changeName(root.name)+"_copy"));
			FileOutputFormat.setOutputPath(job, new Path("/user/training/FYP/dummyoutput_copy"));
			job.setInputFormatClass(SequenceFileInputFormat.class);
			job.setOutputFormatClass(SequenceFileOutputFormat.class);
			
			if (fs.exists(new Path("/user/training/FYP/dummyoutput_copy")))
				fs.delete(new Path("/user/training/FYP/dummyoutput_copy"), true);
	...
	private void evidenceCollection(JTNode root) throws IOException, InterruptedException, ClassNotFoundException {
		...
		if(len > 0){
			...
			SequenceFile.Reader reader = null;
			SequenceFile.Writer writer = SequenceFile.createWriter(fs,conf,new Path("/user/training/FYP/sample-input/file.seq"),CliqueNode.class,CliqueNode.class);
			FileStatus[] fss = fs.listStatus(new Path("/user/training/FYP/"+changeName(root.name)+"_copy"),new PathFilter() {
								  @Override
								  public boolean accept(Path path) {
								    return path.getName().endsWith(".seq") || path.getName().startsWith("part");
			...
			for(int i=0;i<len;i++){
				fss = fs.listStatus(new Path("/user/training/FYP/"+changeName(children.get(i).name)+"_copy"),new PathFilter() {
						  @Override
						  public boolean accept(Path path) {
							  return path.getName().endsWith(".seq") || path.getName().startsWith("part");
						  }
			...
			FileInputFormat.addInputPath(job, new Path("/user/training/FYP/sample-input"));
			FileOutputFormat.setOutputPath(job, new Path("/user/training/FYP/"+changeName(root.name)+"_copy"));
			job.setInputFormatClass(SequenceFileInputFormat.class);
			job.setOutputFormatClass(SequenceFileOutputFormat.class);
			
			if (fs.exists(new Path("/user/training/FYP/"+changeName(root.name)+"_copy")))
				fs.delete(new Path("/user/training/FYP/"+changeName(root.name)+"_copy"), true);
